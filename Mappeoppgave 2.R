...
covid_deaths %>% #henter fra datasett
  ...
  scale_x_continuous(labels = scales::percent, limits = c(0.45, 0.8), #x-akse vises i prosent.
                     breaks = seq(0.45, 0.8, by = 0.05))+ #bestemmer skala og oppdeling av x-akse.
  ...
  ylab('20. avg monthly deaths per 100,000')+ #tekst ved y-akse
...
# I dette tilfellet betyr det at når vaksinasjonsraten er lik 0%, så dør 31,15 av 100,000 av covid i tidsperioden dataen er hentet fra. 
# Dette er ikke emperiske tall, men et anslag gjort av regresjons-modellen.
#
# Under fully_vaccinated_pct_of_pop" vil det stå igjen ett tall mellom -100 og 100 som viser "korrelasjosstyrke" mellom variablene.
# Jo nærmere tallet er -100 eller 100 jo sterkere er korrelasjonen
# Vårt tall er -36.66. Dette viser en negativ korrelasjon mellom antall døde og prosentandell vaksinerte.
# Altså, når færre dør, så øker prosentandelen vaksinerte. Eller omvendt, som vil være den mest logiske slutningen i dette tilfellet.
# Flere vaksinerte betyr færre døde.
...
covid_deaths %>% 
  ...
  ylab('20. avg monthly deaths per 100,000')+
